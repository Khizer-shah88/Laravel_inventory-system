@extends('layouts.app')

@section('page_title', 'Sale Invoices')

@push('styles')
<style>
/* ===============================
   GLOBAL COMPACT SETTINGS
================================ */
body {
    font-size: 0.85rem;
}

.form-control,
.form-select,
.input-group-text,
.btn {
    font-size: 0.8rem;
}

/* Remove focus glow */
.form-control:focus,
.form-select:focus,
.btn:focus {
    box-shadow: none !important;
    border-color: #adb5bd;
}

/* ===============================
   COMPACT CARD (MASTER INFO)
================================ */
.card-compact {
    border: 1px solid #dee2e6;
}

.card-compact .card-header {
    padding: 6px 12px;
    font-size: 0.85rem;
    font-weight: 600;
}

.card-compact .form-control,
.card-compact .input-group-text {
    height: 30px;
    padding: 3px 8px;
}

.card-compact .input-group-text {
    min-width: 110px;
    background: #f8f9fa;
    font-weight: 600;
}

/* ===============================
   TABLE – ITEM ENTRY
================================ */
.table-compact th,
.table-compact td {
    padding: 4px;
    vertical-align: middle;
}

.table-compact thead {
    background: #212529;
    color: #fff;
}

.table-compact label {
    font-size: 0.7rem;
    font-weight: 600;
    margin-bottom: 2px;
    color: #495057;
    white-space: nowrap;
}

.table-compact .form-control {
    height: 30px;
    padding: 3px 6px;
}

.table-compact input:disabled {
    background: #f1f3f5;
    color: #212529;
    font-weight: 600;
    text-align: center;
}

/* Dropdown lists */
.dropdown-menu {
    font-size: 0.8rem;
}

/* ===============================
   BUTTONS
================================ */
.btn {
    height: 32px;
    padding: 4px 10px;
    font-weight: 600;
}

.add-item-btn {
    width: 32px;
    height: 32px;
    padding: 0;
}

/* ===============================
   FIXED STICKY FOOTER (BOTTOM)
================================ */
.invoice-footer {
    position: fixed;
    bottom: 0;
    left: 0;
    width: 100%;
    background: #ffffff;
    border-top: 1px solid #dee2e6;
    padding: 6px 10px;
    z-index: 1050;
}


/* ===============================
   FOOTER SUMMARY TABLE
================================ */
.invoice-footer table {
    margin-bottom: 0;
}

.invoice-footer th {
    font-size: 0.7rem;
    text-transform: uppercase;
    background: #f8f9fa;
    padding: 4px;
    text-align: center;
}

.invoice-footer td {
    padding: 4px;
    text-align: center;
    vertical-align: middle;
}

.invoice-footer input {
    height: 30px;
    padding: 3px 6px;
    text-align: right;
}

.invoice-footer .amount {
    font-weight: 700;
    font-size: 0.9rem;
}

.invoice-footer .net-amount {
    font-weight: 800;
    font-size: 1rem;
    color: #0d6efd;
}

/* ===============================
   NUMERIC ALIGNMENT
================================ */
.text-end {
    text-align: right !important;
}

.text-center {
    text-align: center !important;
}
.input-group-text.fixed-width {
    width: 25%;          /* Fixed width as requested */
    text-align: right;   /* Optional: right-align text */
    min-width: 100px;    /* Optional: set a minimum width */
}

</style>

@endpush
@section('content')
    
    <div class="container-fluid">

            <!-- Master Table -->



    <div class="card card-compact rounded-0">
    <div class="card-header bg-primary text-white rounded-0">
        <i class="fas fa-table me-2"></i>Master Information
    </div>

    <div class="row g-2 p-2">

        <!-- LEFT COLUMN -->
        <div class="col-md-6">

            <div class="input-group input-group-sm">
                <span class="input-group-text">Header Code</span>
                <input type="number" class="form-control" id="headerCode">
            </div>

            <div class="input-group input-group-sm">
                <span class="input-group-text">Account ID</span>
                <input type="number" class="form-control" id="accountId">
            </div>

            <div class="input-group input-group-sm">
                <span class="input-group-text">Account Name</span>
                <input list="accountsList"
                       id="accountName"
                       class="form-control"
                       placeholder="Search account..."
                       autocomplete="off"
                       onblur="checkAccountName()">
            </div>
            <span id="accountNameError" class="text-danger small"></span>

            <datalist id="accountsList">
                @foreach ($accounts as $account)
                    <option
                        value="{{ $account->AccountName }}"
                        data-HeaderCode="{{ $account->HeaderCode }}"
                        data-AccountId="{{ $account->AccountId }}"
                        data-CustomerType="{{ $account->CustomerType }}"
                        data-Town="{{ $account->Town }}">
                        {{ $account->Town }}
                    </option>
                @endforeach
            </datalist>

            <div class="input-group input-group-sm">
                <span class="input-group-text">Town</span>
                <input type="text" class="form-control" id="town">
            </div>

        </div>

        <!-- RIGHT COLUMN -->
        <div class="col-md-6">

            <div class="input-group input-group-sm">
                <span class="input-group-text">Invoice Date</span>
                <input type="date"
                       class="form-control"
                       id="invDate"
                       value="{{ date('Y-m-d') }}">
            </div>

            <div class="input-group input-group-sm">
                <span class="input-group-text">Invoice No</span>
                <input type="text" class="form-control" id="invNo" name="invoiceId" readonly>
                
            </div>

            <div class="input-group input-group-sm">
                <span class="input-group-text">Customer Type</span>
                <input type="text" class="form-control" id="customerType">
            </div>

        </div>

    </div>
</div>





<table class="table table-bordered table-compact align-middle mb-0">
    <thead>
        <tr>

            <th>
                <label>Item Code</label>
                <div class="position-relative">
                    <input type="text"
                           class="form-control text-center"
                           id="itemCode"
                           placeholder="Code"
                           autocomplete="off">
                    <div id="ItemCodeList"
                         class="dropdown-menu w-100"
                         style="max-height:220px; overflow:auto; display:none;">
                    </div>
                </div>
            </th>

            <th style="width:30%;">
                <label>Item Name</label>
                <div class="position-relative">
                    <input type="text"
                           class="form-control text-center"
                           id="itemName"
                           placeholder="Item Name"
                           autocomplete="off">
                    <div id="ItemNameList"
                         class="dropdown-menu w-100"
                         style="max-height:220px; overflow:auto; display:none;">
                    </div>
                </div>
            </th>

            <th>
                <label>Pkt Size</label>
                <input type="number" class="form-control text-center" id="packetSize" value="1" min="1" readonly>
            </th>

            <th>
                <label>Pkt Qty</label>
                <input type="number" class="form-control text-center" id="packetQty" value="0" min="0">
            </th>

            <th>
                <label>Qty</label>
                <input type="number" class="form-control text-center" id="qty" value="0" min="0">
            </th>

            <th>
                <label>Total Qty</label>
                <input type="text" class="form-control text-center" id="totalQty" value="0" disabled>
            </th>

            <th>
                <label>Rate</label>
                <input type="number" class="form-control text-end" id="rate" value="0.00" min="0" step="0.01">
            </th>
            <th class="d-none">
                <label>Pprice</label>
                <input type="number" class="form-control text-end" id="prate" value="0.00" min="0" step="0.01">
            </th>

            <th>
                <label>Pkt Rate</label>
                <input type="number" class="form-control text-end" id="packetRate" value="0.00" min="0" step="0.01">
            </th>

            <th class="total-col">
                <label>Total</label>
                <input type="text" class="form-control text-end" id="total" value="0.00" disabled>
            </th>

            <th class="text-center">
                <label class="d-block">&nbsp;</label>
                <button id="addItemBtn" class="btn btn-primary add-item-btn">
                    <i class="fas fa-plus"></i>
                </button>
                
            </th>

        </tr>
    </thead>

    <tbody id="itemsTableBody"></tbody>
</table>



<!-- Sticky Footer -->
<div class="invoice-footer shadow-sm">
    <div class="container-fluid">
        <div class="row align-items-center g-2">

            <!-- LEFT: ACTION BUTTONS -->
            <div class="col-md-6">
                <div class="d-flex gap-2">

                    <button id="saveInvoiceBtn" type="button" class="btn btn-success">
                        <i class="fas fa-save me-1"></i>Save/Close <span class="shortcut-key">F10</span>
                    </button>


                    <button id="newInvoiceBtn" class="btn btn-primary">
                        <i class="fas fa-plus me-1"></i>
                        New <span class="shortcut-key">Ctrl+N</span>
                    </button>


<a href="javascript:void(0)" class="btn btn-sm btn-warning" onclick="printInvoiceA4()">
    <i class="fa fa-print text-primary"></i>Print A4
</a>

<a href="/SaleInvoice" class="btn btn-sm btn-danger">Cancle</a>

                </div>
            </div>

            <!-- RIGHT: SUMMARY -->
            <div class="col-md-6">
                <table class="table table-bordered table-sm text-center">
                    <thead>
                        <tr>
                            <th>Invoice Amount</th>
                            <th>Discount</th>
                            <th class="d-none">Cash Received</th>
                            <th>Net Amount</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td class="amount">
                                <span id="grandTotal">0.00</span>
                            </td>

                            <td>
                                <input type="number"
                                       id="cashDiscount"
                                       value="0"
                                       min="0"
                                       step="0.01">
                            </td>

                            <td class="d-none">
                                <input type="number"
                                       id="cashReceived"
                                       value="0"
                                       min="0"
                                       step="0.01">
                            </td>

                            <td class="net-amount">
                                <span id="netAmount">0.00</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>

        </div>
    </div>
</div>

</br>



        </div>


    
@push('scripts')

<script>
function printInvoiceA4() {

    saveInvoice().then(response => {

        // assume controller returns: { invNo: 123 }
        const invNo = response.invNo;

        if (!invNo) {
            alert('Invoice not saved');
            return;
        }

        document.getElementById('invNo').value = invNo;

        const url = "{{ url('/SaleInvoice/showa4') }}/" + invNo;
        window.open(url, '_blank');

    }).catch(err => {
        console.error(err);
        alert('Error saving invoice');
    });
}


function checkAccountName() {
    const input = document.getElementById('accountName');

    if (input.value.trim() === "") {
        alert("Fill accountName");

        // Delay the focus to prevent stuck loop
        setTimeout(() => {
            input.focus();
        }, 0);
    }
}


  // Select all input fields
  const inputs = document.querySelectorAll('input');
  const addItemBtn = document.getElementById('addItemBtn');
  
 

  inputs.forEach(input => {
    // Highlight button when input is focused
    input.addEventListener('focus', () => {
      addItemBtn.style.backgroundColor = '#cdcdcd'; // yellow highlight
      addItemBtn.style.color = '#000'; // optional text color
    });

    // Revert button when input loses focus
    input.addEventListener('blur', () => {
      addItemBtn.style.backgroundColor = '#ffeb3b'; // remove highlight
      addItemBtn.style.color = ''; // revert text color
    });
  });


    $('#packetQty').on('keypress', function (e) {
    if (e.which === 13) { // 13 = Enter key
        e.preventDefault(); // prevent form submit if inside a form
        $('#qty').focus().select();;
    }
});


$('#itemCode').on('keypress', function (e) {
    if (e.which === 13) { // 13 = Enter key
        e.preventDefault(); // prevent form submit if inside a form
        $('#itemName').focus();
    }
});


$('#rate').on('keypress', function (e) {
    if (e.which === 13) { // 13 = Enter key
        e.preventDefault(); // prevent form submit if inside a form
        $('#addItemBtn').focus();
    }
});

$('#qty').on('keypress', function (e) {
    if (e.which === 13) { // 13 = Enter key
        e.preventDefault(); // prevent form submit if inside a form
        $('#rate').focus().select();
    }
});





    document.getElementById('accountName').addEventListener('input', function() {
    const inputVal = this.value.trim();
    const options = document.querySelectorAll('#accountsList option');

    // Reset fields if input is empty or invalid
    document.getElementById('headerCode').value = '';
    document.getElementById('accountId').value = '';
    document.getElementById('town').value = '';
    document.getElementById('customerType').value = '';

    options.forEach(option => {
        if (option.value === inputVal) {
            document.getElementById('headerCode').value = option.dataset.headercode;
            document.getElementById('accountId').value = option.dataset.accountid;
            document.getElementById('town').value = option.dataset.town;
            document.getElementById('customerType').value = option.dataset.customertype;
            document.getElementById('itemCode').value = '';
            $('#itemCode').focus();
        }
    });
});

$('#itemCode').on('change', function () {
    let code = $(this).val();
    let customerType = $('#customerType').val();


console.log("Barcode:", code);
console.log("Customer Type:", customerType);



    $.ajax({
        url: '/items/' + code + '/details/' + customerType,
        type: 'GET',
        success: function (data) {
            $('#itemName').val(data.ItemName);
            $('#packetSize').val(data.PacketSize);
            $('#rate').val(data.Rate);
            $('#prate').val(data.Prate);
            $('#packetRate').val(data.packetRate);
            $('#qty').val(1);
    
    
        const rate = parseFloat($('#rate').val()) || 0;
      const qty  = parseFloat($('#qty').val())  || 0;
      const total = rate * qty;

      // set total field (create one if you don't have)
      $('#total').val(total.toFixed(2)); // adjust formatting as needed
      $('#qty').focus().select();
            
        },
        error: function () {
            alert("Item not found");
        }
    });
});




function printInvoice() {
    const invoiceId = document.getElementById('invNo').value;

    if (!invoiceId) {
        alert('Invoice ID is missing.');
        return;
    }

    // Build the Laravel route dynamically (assuming typical REST route)
    const url = `/SaleInvoice/${invoiceId}`;  // Update if your route is different
    openInvoice(url);
}

function openInvoice(url) {
    window.open(url, '_blank');
}




    
        // ========= Fast dropdown: Items =========
        const $itemInput = $("#itemName");
        const $itemMenu  = $("#ItemNameList");
        const $itemCodeInput = $("#itemCode");
        const $rateInput = $("#rate");
        const $prateInput = $("#prate");
        const $packetSizeInput = $("#packetSize");

        function renderItemMenu(data) {
    $itemMenu.empty();
    if (!data || !data.length) {
        $itemMenu.append('<div class="dropdown-item text-muted">No results found</div>');
        return;
    }

    data.forEach(item => {
        const $opt = $('<button type="button" class="dropdown-item text-start"></button>')
            .data('record', item);

        const $content = $('<div class="item-option"></div>');

        // First line: ItemName
        $content.append($('<div class="item-name"></div>').text(item.ItemName));

const customerType = $('#customerType').val();
// Select price based on customer type
let price = "0.00";


if (customerType === "Retail") {
    price = item.USalprice || "0.00";
} 
else if (customerType === "Whole Sale") {
    price = item.WUprice || "0.00";
}
else if (customerType === "Distributor") {
    price = item.DUprice || "0.00";
}


        // Build second line display
        const itemCode = item.ItemCode || 'N/A';
        const packetSize = item.PacketSize || 'N/A';

        const detailsText = `${itemCode} | Size: ${packetSize} | Price: ${price}`;
        $content.append($('<div class="item-details"></div>').text(detailsText));

        $opt.append($content);
        $itemMenu.append($opt);
    });

    $itemMenu.children().first().addClass('active');
}


        function fetchItems(term) {
            if (term.length < 2) { 
                $itemMenu.removeClass('show').hide(); 
                return; 
            }
               // Get customer type from the page
            const customerType = $('#customerType').val();  
            // Show loading indicator
            $itemMenu.html('<div class="dropdown-item text-muted">Searching...</div>').show();
            
            // Fetch items from server
            $.ajax({
                url: "{{ route('get.items') }}",
                method: "GET",
                data: { 
                    ItemName: term,
                    _token: "{{ csrf_token() }}"
                },
                success: function(data) {
                    renderItemMenu(data);
                    if (data && data.length) {
                        $itemMenu.addClass('show').show();
                    } else {
                        $itemMenu.removeClass('show').hide();
                    }
                },
                error: function(xhr, status, error) {
                    console.error("Error fetching items:", error);
                    $itemMenu.html('<div class="dropdown-item text-danger">Error loading items</div>');
                }
            });
        }

        $itemInput.on('input', function(){ 
            fetchItems(this.value.trim()); 
        });

        $itemMenu.on('click', '.dropdown-item', function(e){
            e.preventDefault();
            const r = $(this).data('record'); 
            if(!r) return;
            
            $itemInput.val(r.ItemName);
            $itemCodeInput.val(r.ItemCode || '');
            $rateInput.val(r.USalprice || '0');
            $prateInput.val(r.UPurprice || '0');
            $packetSizeInput.val(r.PacketSize || '1');
            $itemMenu.removeClass('show').hide();
            
            $itemInput.val(r.ItemName);
            $itemCodeInput.val(r.ItemCode || '');
            $packetSizeInput.val(r.PacketSize || '1');
            
            // Get customer type from the page
            const customerType = $('#customerType').val();
            
            // Determine correct rate
            let rate = "0";
            if (customerType === "Retail") {
                rate = r.USalprice || "0";
            } else if (customerType === "Whole Sale") {
                rate = r.WUprice || "0";
            } else if (customerType === "Distributor") {
                rate = r.DUprice || "0";
            }
            
            $rateInput.val(rate);

            
            // Close menu
            $itemMenu.removeClass('show').hide();

            
            // Trigger calculations after setting values
            const event = new Event('input', { bubbles: true });
            $rateInput[0].dispatchEvent(event);
            $packetSizeInput[0].dispatchEvent(event);
            
            // Focus on packet quantity
            $('#qty').focus().select();
        });

        $(document).on('click', function(e){
            if(!$(e.target).closest('#itemName, #ItemNameList').length){ 
                $itemMenu.removeClass('show').hide(); 
            }
        });

        // Keyboard navigation
        $itemInput.on('keydown', function(e){
            if(!$itemMenu.hasClass('show') || !$itemMenu.is(':visible')) return;
            
            const $items = $itemMenu.find('.dropdown-item');
            let $active = $items.filter('.active');
            
            if(e.key === 'ArrowDown'){
                e.preventDefault();
                if($active.length){ 
                    const $next = $active.next('.dropdown-item'); 
                    if($next.length){ 
                        $active.removeClass('active'); 
                        $next.addClass('active');
                        ensureVisible($itemMenu, $next);
                    }
                } else { 
                    $items.first().addClass('active');
                    ensureVisible($itemMenu, $items.first());
                }
            } else if(e.key === 'ArrowUp'){
                e.preventDefault();
                if($active.length){ 
                    const $prev = $active.prev('.dropdown-item'); 
                    if($prev.length){ 
                        $active.removeClass('active'); 
                        $prev.addClass('active');
                        ensureVisible($itemMenu, $prev);
                    }
                }
            } else if(e.key === 'Enter'){
                e.preventDefault(); 
                if($active.length){ 
                    $active.trigger('click'); 
                }
            } else if(e.key === 'Escape'){
                $itemMenu.removeClass('show').hide();
            }
        });
        
        function ensureVisible($container, $element) {
            const containerHeight = $container.height();
            const containerScrollTop = $container.scrollTop();
            const elementTop = $element.position().top;
            const elementHeight = $element.outerHeight();
            
            if (elementTop < 0) {
                $container.scrollTop(containerScrollTop + elementTop);
            } else if (elementTop + elementHeight > containerHeight) {
                $container.scrollTop(containerScrollTop + (elementTop + elementHeight - containerHeight));
            }
        }




    //Account Name
    // Account dropdown functionality
const $input = $('#accountName');
const $list = $('#AccountNameList'); // should be <div class="dropdown-menu" id="AccountNameList"></div>
let selectedIndex = -1;



function openDropdown() {
    selectedIndex = -1;
    $list.addClass('show').show();
}


function closeDropdown() {
    selectedIndex = -1;
    $list.removeClass('show').hide().empty();
}

function renderAccountMenu(accounts) {
    $list.empty();
    accounts.forEach((account) => {
        const $item = $('<a href="#" class="dropdown-item"></a>').text(account.AccountName);
        $item.data('account', account);
        $item.on('click', function(e) {
            e.preventDefault();
            selectAccount($(this));
        });
        $list.append($item);
    });
}



function highlightItem(index) {
    const items = $list.children('.dropdown-item');
    if (!items.length) return;

    items.removeClass('active');

    if (index < 0) {
        selectedIndex = -1;
        return;
    }

    if (index >= items.length) {
        selectedIndex = items.length - 1;
    } else {
        selectedIndex = index;
    }

    const $current = items.eq(selectedIndex);
    $current.addClass('active');

    const listTop = $list.scrollTop();
    const listBottom = listTop + $list.outerHeight();
    const itemTop = $current.position().top + listTop;
    const itemBottom = itemTop + $current.outerHeight();

    if (itemBottom > listBottom) {
        $list.scrollTop(itemBottom - $list.outerHeight());
    } else if (itemTop < listTop) {
        $list.scrollTop(itemTop);
    }
}

function positionDropdown() {
    const inputOffset = $input.offset();
    $list.css({
        position: 'absolute',
        top: inputOffset.top + $input.outerHeight(),
        left: inputOffset.left,
        width: $input.outerWidth(),
        zIndex: 1000
    });
}



$input.on('keydown', function(e) {
    const items = $list.children('.dropdown-item');
    if (!$list.is(':visible') || !items.length) return;

    switch (e.key) {
        case 'ArrowDown':
            e.preventDefault();
            highlightItem(selectedIndex + 1);
            break;
        case 'ArrowUp':
            e.preventDefault();
            highlightItem(selectedIndex - 1);
            break;
        case 'Enter':
            e.preventDefault();
            if (selectedIndex >= 0) {
                selectAccount(items.eq(selectedIndex));
            } else if (items.length) {
                selectAccount(items.eq(0)); // fallback to first
            }
            break;
        case 'Escape':
            closeDropdown();
            break;
    }
});


// Focus first
$input.focus();


        document.addEventListener('DOMContentLoaded', function() {
            // Array to store items
            let items = [];
            let currentInvoiceId = null;
            let isSaved = false;
            
            // DOM Elements
            const invNoInput = document.getElementById('invNo');
            const invoiceIdInput = document.getElementById('invNo');
            const invoiceStatus = document.getElementById('invoiceStatus');
            const saveInvoiceBtn = document.getElementById('saveInvoiceBtn');
            const newInvoiceBtn = document.getElementById('newInvoiceBtn');
            const printInvoiceBtn = document.getElementById('printInvoiceBtn');
            const addItemBtn = document.getElementById('addItemBtn');
            const cashDiscountInput = document.getElementById('cashDiscount');
            const cashReceivedInput = document.getElementById('cashReceived');
            const notesInput = document.getElementById('notes');
            
            // Get CSRF token safely
            function getCsrfToken() {
                const metaTag = document.querySelector('meta[name="csrf-token"]');
                return metaTag ? metaTag.getAttribute('content') : '';
            }
            
            // Safe element access with null checking
            function getElementSafe(id) {
                const element = document.getElementById(id);
                if (!element) {
                    console.error(`Element with ID '${id}' not found`);
                    return null;
                }
                return element;
            }
            
            // Initialize event listeners only if elements exist
            function initializeEventListeners() {
                if (addItemBtn) addItemBtn.addEventListener('click', addItem);
                if (saveInvoiceBtn) saveInvoiceBtn.addEventListener('click', saveInvoice);
                if (newInvoiceBtn) newInvoiceBtn.addEventListener('click', createNewInvoice);
                
                // Update summary when values change
                if (cashDiscountInput) cashDiscountInput.addEventListener('input', updateSummary);
                if (cashReceivedInput) cashReceivedInput.addEventListener('input', updateSummary);
                
                // Auto-calculate when packet fields change
                const packetSize = getElementSafe('packetSize');
                const packetQty = getElementSafe('packetQty');
                const qty = getElementSafe('qty');
                const rate = getElementSafe('rate');
                
                if (packetSize) packetSize.addEventListener('input', calculateTotal);
                if (packetQty) packetQty.addEventListener('input', calculateTotal);
                if (qty) qty.addEventListener('input', calculateTotal);
                if (rate) rate.addEventListener('input', calculateTotal);
            }
            
            // Keyboard shortcuts - FIXED F10 HANDLING
            document.addEventListener('keydown', function(e) {
                // F10 to save invoice
                if (e.key === 'F10' || e.keyCode === 121) {
                    e.preventDefault();
                    saveInvoice();
                    return false;
                }
                
                // Ctrl+N for new invoice
                if (e.ctrlKey && (e.key === 'n' || e.key === 'N')) {
                    e.preventDefault();
                    createNewInvoice();
                    return false;
                }
                

            });
            
            // Function to calculate total quantity and total automatically
            function calculateTotal() {
                const packetSize = getElementSafe('packetSize');
                const packetQty = getElementSafe('packetQty');
                const qty = getElementSafe('qty');
                const rate = getElementSafe('rate');
                const totalQtyInput = getElementSafe('totalQty');
                const totalInput = getElementSafe('total');
                
                if (!packetSize || !packetQty || !qty || !rate || !totalQtyInput || !totalInput) return 0;
                
                const packetSizeVal = parseInt(packetSize.value) || 0;
                const packetQtyVal = parseInt(packetQty.value) || 0;
                const qtyVal = parseInt(qty.value) || 0;
                const rateVal = parseFloat(rate.value) || 0;
                
                const totalQty = (packetSizeVal * packetQtyVal) + qtyVal;
                const total = totalQty * rateVal;
                
                totalQtyInput.value = totalQty;
                totalInput.value = total.toFixed(2);
                console.log('yes it is')
                return total;
            }
            
            $('#packetRate').on('input', function () {

    const packetSize = parseFloat($('#packetSize').val()) || 0;
    const packetRate = parseFloat($(this).val()) || 0;

    let rate = 0;

    if (packetRate > 0) {
        rate = packetRate / packetSize;
    }

    $('#rate').val(rate.toFixed(2));

    // Trigger your existing total calculation
    $('#rate').trigger('input');

    calculateTotal();

});
$('#rate').on('input', function () {

    const rate  = parseFloat($('#rate').val())  || 0;
    const prate = parseFloat($('#prate').val()) || 0;

    if (rate > 0 && prate > 0 && rate < prate) {
        alert('آپ نقصان میں مال فروخت کر رہے ہیں');
    }
});

            // Function to add item
            function addItem() {
                const itemCode = getElementSafe('itemCode');
                const itemName = getElementSafe('itemName');
                const packetSize = getElementSafe('packetSize');
                const packetQty = getElementSafe('packetQty');
                const qty = getElementSafe('qty');
                const rate = getElementSafe('rate');
                const packetRate = getElementSafe('packetRate');
                const totalInput = getElementSafe('total');
                
                if (!itemCode || !itemName || !packetSize || !packetQty || !qty || !rate || !totalInput) return;
                
                const itemCodeVal = itemCode.value.trim();
                const itemNameVal = itemName.value.trim();
                const packetSizeVal = parseInt(packetSize.value) || 0;
                const packetQtyVal = parseInt(packetQty.value) || 0;
                const qtyVal = parseInt(qty.value) || 0;
                const rateVal = parseFloat(rate.value) || 0;
                const packetRateVal = parseFloat(packetRate.value) || 0;
                const totalQtyVal = (packetSizeVal * packetQtyVal) + qtyVal;
                const totalVal = totalQtyVal * rateVal;
                
                if (!itemCodeVal || !itemNameVal) {
                    alert('Please enter Item Code and Item Name');
                    return;
                }
                
                if (totalQtyVal <= 0) {
                    alert('Total quantity must be greater than 0');
                    return;
                }
                
                if (rateVal <= 0) {
                    alert('Rate must be greater than 0');
                    return;
                }
                
                // Add to items array at the beginning (so it shows on top)
                items.unshift({
                    itemCode: itemCodeVal,
                    itemName: itemNameVal,
                    packetSize: packetSizeVal,
                    packetQty: packetQtyVal,
                    qty: qtyVal,
                    totalQty: totalQtyVal,
                    rate: rateVal,
                    packetRate: packetRateVal,
                    total: totalVal
                });
                
                // Update table
                updateItemsTable();
                
                // Clear input fields
                itemCode.value = '';
                itemName.value = '';
                packetSize.value = 1;
                packetQty.value = 0;
                qty.value = 0;
                rate.value = '';
                packetRate.value = '';
                
                const totalQtyInput = getElementSafe('totalQty');
                if (totalQtyInput) totalQtyInput.value = 0;
                if (totalInput) totalInput.value = '0.00';
                
                // Focus back to item code
                itemCode.focus();
            }
            
            // Function to update items table with editable fields
           // Function to recalculate a single row and update totals
        function recalcRow(row, index) {
            const item = items[index];
        
            // Get updated values from the inputs
            const packetSize = parseInt(row.querySelector('input[data-field="packetSize"]').value) || 0;
            const packetQty  = parseInt(row.querySelector('input[data-field="packetQty"]').value) || 0;
            const qty        = parseInt(row.querySelector('input[data-field="qty"]').value) || 0;
            const rate       = parseFloat(row.querySelector('input[data-field="rate"]').value) || 0;
        
            // Update the item object
            item.packetSize = packetSize;
            item.packetQty  = packetQty;
            item.qty        = qty;
            item.rate       = rate;
        
            // Recalculate totals
            item.totalQty = (packetSize * packetQty) + qty;
            item.total = item.totalQty * rate;
        
            // Update row display
            row.querySelector('.totalQty').textContent = item.totalQty;
            row.querySelector('.rowTotal').textContent = item.total.toFixed(2);
        
            // Update grand total
            updateSummary();
        }

        // Updated function to render table
        function updateItemsTable() {
            const tableBody = getElementSafe('itemsTableBody');
            if (!tableBody) return;
        
            tableBody.innerHTML = '';
        
            if (items.length === 0) {
                tableBody.innerHTML = `
                    <tr>
                        <td colspan="9" class="text-center text-muted">No items added</td>
                    </tr>`;
                return;
            }
        
            items.forEach((item, index) => {
                const row = document.createElement('tr');
                row.className = 'item-row';
                row.dataset.index = index;
        
                row.innerHTML = `
                    <td>${item.itemCode}</td>
                    <td>${item.itemName}</td>
                    <td><input type="number" class="form-control form-control-sm editable" 
                               value="${item.packetSize}" min="1" data-field="packetSize" readonly></td>
                    <td><input type="number" class="form-control form-control-sm editable" 
                               value="${item.packetQty}" min="0" data-field="packetQty"></td>
                    <td><input type="number" class="form-control form-control-sm editable" 
                               value="${item.qty}" min="0" data-field="qty"></td>
                    <td class="totalQty">${item.totalQty}</td>
                    <td><input type="number" class="form-control form-control-sm editable" 
                               value="${item.rate.toFixed(2)}" min="0" step="0.01" data-field="rate"></td>
                    
                    <td><input type="number" class="form-control form-control-sm editable" 
                               value="${item.packetRate.toFixed(2)}" min="0" step="0.01" data-field="packetRate"></td>
        
                    <td class="rowTotal">${item.total.toFixed(2)}</td>
                                <td>
                        <button class="btn btn-sm btn-danger remove-item">
                            <i class="fas fa-trash"></i>
                        </button>
                    </td>
                `;
        
                tableBody.appendChild(row);
        
                // Attach event listener to inputs using the separate function
                const editableInputs = row.querySelectorAll('.editable');
                editableInputs.forEach(input => {
                    input.addEventListener('input', () => recalcRow(row, index));
                });
        
                // Remove button
                const removeBtn = row.querySelector('.remove-item');
                removeBtn.addEventListener('click', function () {
                    items.splice(index, 1);
                    updateItemsTable();
                });
            });
        
            // Update grand total after rendering
            updateSummary();
        }

            
            // Function to update summary
        function updateSummary() {
            const grandTotalElement = document.getElementById('grandTotal');
            const netAmountElement  = document.getElementById('netAmount');
            const cashDiscountInput = document.getElementById('cashDiscount');
            const cashReceivedInput = document.getElementById('cashReceived');
        
            if (!grandTotalElement || !netAmountElement ||
                !cashDiscountInput || !cashReceivedInput) return;
        
            const grandTotal = items.reduce((sum, item) => sum + (item.total || 0), 0);
            const cashDiscount = parseFloat(cashDiscountInput.value) || 0;
            const cashReceived = parseFloat(cashReceivedInput.value) || 0;
        
            const netAmount = grandTotal - cashDiscount - cashReceived;
        
            grandTotalElement.textContent = grandTotal.toFixed(2);
            netAmountElement.textContent  = netAmount.toFixed(2);
        }
        

            
            // Function to save invoice
function saveInvoice() {
    console.log('save click');

    const saveInvoiceBtn = document.getElementById('saveInvoiceBtn');
    const cashDiscountInput = document.getElementById('cashDiscount');
    const cashReceivedInput = document.getElementById('cashReceived');

    if (items.length === 0) {
        alert('Please add at least one item');
        return;
    }

    const headerCode = document.getElementById('headerCode');
    const accountId = document.getElementById('accountId');
    const accountName = document.getElementById('accountName');
    const invDate = document.getElementById('invDate');
    const town = document.getElementById('town');
    const invNoInput = document.getElementById('invNo');

    if (!headerCode || !accountId || !accountName || !invDate || !town) return;

    const formData = {
        invoiceId: invNoInput && invNoInput.value ? parseInt(invNoInput.value) : null,
        invDate: invDate.value,
        headerCode: headerCode.value,
        accountId: accountId.value,
        accountName: accountName.value,
        town: town.value,
        cashDiscount: parseFloat(cashDiscountInput.value) || 0,
        cashReceived: parseFloat(cashReceivedInput.value) || 0,
        items: items
    };

    saveInvoiceBtn.disabled = true;

    fetch('/sale-invoice', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': getCsrfToken()
        },
        body: JSON.stringify(formData)
    })
    .then(res => res.json())
    .then(data => {
        if (!data.success) {
            alert('Error saving invoice: ' + data.message);
            return;
        }

        // Set invoice number
        if (invNoInput) {
            invNoInput.value = data.invNo;
        }

        // Ask for print
        const printConfirm = confirm(
            'Invoice saved successfully.\nInvoice No: ' + data.invNo + '\n\nDo you want to print this invoice?'
        );

        if (printConfirm) {

        const url = "{{ url('/SaleInvoice/showa4') }}/" + data.invNo;
        window.open(url, '_blank');


        } else {
            window.location.href = '/SaleInvoice';
        }
    })
    .catch(err => {
        console.error(err);
        alert('Something went wrong while saving invoice');
    })
    .finally(() => {
        saveInvoiceBtn.disabled = false;
    });
}


            
    // Function to create a new invoice
    function createNewInvoice() {
        if (confirm('Create a new invoice? Any unsaved changes will be lost.')) {
            // Reset form
            const headerCode = getElementSafe('headerCode');
            const accountId = getElementSafe('accountId');
            const accountName = getElementSafe('accountName');
            const town = getElementSafe('town');
            
            if (headerCode) headerCode.value = '';
            if (accountId) accountId.value = '';
            if (accountName) accountName.value = '';
            if (town) town.value = '';
            if (cashDiscountInput) cashDiscountInput.value = 0;
            if (cashReceivedInput) cashReceivedInput.value = 0;
            if (notesInput) notesInput.value = '';
            
            // Reset items
            items = [];
            updateItemsTable();
            
            // Reset invoice ID
            currentInvoiceId = null;
            if (invoiceIdInput) invoiceIdInput.value = '';
            if (invNoInput) invNoInput.value = 'New';
            
            // Update status
            isSaved = false;
            if (invoiceStatus) {
                invoiceStatus.textContent = 'DRAFT';
                invoiceStatus.className = 'status-indicator status-draft';
            }
            
            // Disable print button
            if (printInvoiceBtn) printInvoiceBtn.disabled = true;
            
            // Focus on first field
            if (headerCode) headerCode.focus();
        }
    }
            
            
            
            
            // Initialize the application
            initializeEventListeners();
            updateItemsTable();
            updateSummary();
            calculateTotal();
        });
    </script>
    
@endpush
@endsection