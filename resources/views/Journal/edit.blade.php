...
<form action="{{ route('journal.update', $journal->CBID ?? 0) }}" method="POST" onsubmit="return confirmSubmit();">
    @csrf
    @method('PUT')
    ...
    <div class="row mb-3">
        <div class="col-md-3">
            <label>CB Date</label>
            <input type="date" name="CBDate" class="form-control" 
                value="{{ old('CBDate', \Carbon\Carbon::parse($journal->CBDate)->format('Y-m-d')) }}" required>
        </div>
        <div class="col-md-3">
            CBID: {{$journal->CBID}}
        </div>
    ...
    <table class="table table-bordered table-sm" id="journalTable">
        ...
        <tbody>
            @foreach($journalSubs as $sub)
            <tr>
                <td ><input type="number" name="HeaderCode[]" class="form-control form-control-sm headerCode" 
                    value="{{ $sub->HeaderCode }}" readonly></td>
                <td ><input type="number" name="AccountId[]" class="form-control form-control-sm accountId" 
                    value="{{ $sub->AccountId }}" readonly></td>
                <td>
                    <input list="accountsList" name="AccountName[]" class="form-control form-control-sm accountName" 
                        value="{{ $sub->AccountName }}" required>
    ...
    @foreach($accounts as $account)
        <option 
            ...
            {{ $account->Town. '-' .$account->DSF. '-' .$account->AccountType}}
        </option>
    @endforeach
                    </datalist>
                </td>
                ...
                <td><input type="text" name="Description[]" class="form-control form-control-sm" 
                    value="{{ $sub->Description }}"></td>
                <td><input type="number" step="0.01" name="Debit[]" class="form-control form-control-sm debit-amount" 
                    value="{{ $sub->Debit }}"></td>
                <td><input type="number" step="0.01" name="Credit[]" class="form-control form-control-sm credit-amount" 
                    value="{{ $sub->Credit }}"></td>
        ...
        <tfoot>
            ...
            <tr class="{{ $errors->has('balance_error') ? 'table-danger' : 'table-success' }}">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Auto-fill HeaderCode and AccountId in each row
    ...
    document.querySelectorAll('#journalTable tbody tr').forEach(bindAutoFill);
